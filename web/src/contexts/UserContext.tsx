...
import { apiClient, type User, type TeamWithMembership, type UserPreferences, type UserTeamsResponse, type Question } from '../lib/api';
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const [defaultTeam, setDefaultTeam] = useState<TeamWithMembership | null>(null);
    ...
    try {
      ...
      if (mockSSOUser) {
        ...
        try {
          ...
          localStorage.setItem(FAVORITES_STORAGE_KEY, JSON.stringify(teamsResponse.favorites));
  ...
  const refreshUserTeams = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to refresh user teams');
    }
  };
  ...
  const refreshUserQuestions = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to refresh user questions');
    }
  };
  ...
  const updatePreferences = async (preferences: Partial<UserPreferences>) => {
    ...
    try {
      ...
      if (preferences.favoriteTeams) {
        ...
        localStorage.setItem(FAVORITES_STORAGE_KEY, JSON.stringify(preferences.favoriteTeams));
      }
      
      if (preferences.defaultTeamId) {
        const newDefaultTeam = userTeams.find(team => team.id === preferences.defaultTeamId);
  ...
  const toggleTeamFavorite = async (teamId: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to toggle team favorite');
      throw err;
    }
  };
  ...
  const setDefaultTeamAction = async (teamId: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to set default team');
      throw err;
    }
  };

  // Utility functions
  const getUserRoleInTeam = (teamId: string): 'member' | 'admin' | 'owner' | null => {
