...
export function AuditPage({ embedded = false }: AuditPageProps = {}) {
  ...
  const loadAuditLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/admin/audit?${params.toString()}`, {
        credentials: 'include',
        headers: {
  ...
  const exportLogs = async (format: 'csv' | 'json') => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/admin/audit/export?${params.toString()}`, {
        credentials: 'include',
        headers: {
          'x-tenant-id': localStorage.getItem('mock-tenant') || 'default',
          'x-mock-sso-user': localStorage.getItem('mock-sso-user') || ''
  ...
  const getActionColor = (action: string) => {
    ...
    if (action.includes('delete') || action.includes('deactivate')) return 'text-red-600 dark:text-red-400';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              >
                ...
                <option value="question.tag.remove">Remove Tag from Question</option>
              </select>
            </div>
