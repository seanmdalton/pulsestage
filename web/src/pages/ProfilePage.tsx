...
export function ProfilePage() {
  ...
  const teamsWithRoles = userTeams.filter(team => getUserRoleInTeam(team.id) !== 'member');
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
          <div className="flex items-center space-x-4">
            <div className="w-16 h-16 bg-blue-600 text-white rounded-full flex items-center justify-center text-xl font-medium">
              {user.name?.split(' ').map(word => word.charAt(0)).join('').toUpperCase() || user.email.charAt(0).toUpperCase()}
            </div>
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                <span className="text-sm text-gray-500 dark:text-gray-400">
                  {userTeams.length} team{userTeams.length !== 1 ? 's' : ''} • {favorites.length} favorite{favorites.length !== 1 ? 's' : ''}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              {/* Team Memberships */}
              <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-3">Team Memberships</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {userTeams.map((team) => {
                    ...
                    return (
                      <div key={team.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          ...
          {activeTab === 'favorites' && (
            <div className="space-y-6">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Favorite Teams</h2>
              ...
              {favoriteTeams.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {favoriteTeams.map((team) => {
                    ...
                    return (
                      <div key={team.id} className="border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-medium text-yellow-900 dark:text-yellow-300">{team.name}</h4>
                          <div className="flex items-center space-x-1">
                            <svg className="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                        ...
                        {team.description && (
                          <p className="text-sm text-yellow-800 dark:text-yellow-400 mb-2">{team.description}</p>
                        )}
                        <div className="flex items-center justify-between">
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                            role === 'owner' 
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">No favorite teams</h3>
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Start favoriting teams from the team selector to see them here.
                  </p>
                </div>
              )}
            </div>
          ...
          {activeTab === 'questions' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">My Questions</h2>
              ...
              {userQuestions.length > 0 ? (
                <div className="space-y-4">
                  {userQuestions.map((question) => (
                    <div key={question.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      ...
                      {question.status === 'ANSWERED' && question.responseText && (
                        <div className="mt-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span className="text-sm font-medium text-green-800 dark:text-green-300">Response</span>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    You haven't submitted any questions yet. Start by asking a question on any team page.
                  </p>
                </div>
              )}
            </div>
