...
export function UserProfile() {
  ...
  const hasAdminPanelAccess =
    hasGlobalAdminRole || hasModeratorRoleInCurrentTeam
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50">
          <div className="py-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {hasGlobalAdminRole && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 flex-shrink-0">
                        {userTeams.some(
                          (t) => getUserRoleInTeam(t.id) === 'owner'
                        )
                          ? 'Owner'
                          : 'Admin'}
                      </span>
            ...
            {/* Moderator Dashboard - For moderators/admins/owners */}
            {hasAdminPanelAccess && (
              <button
                onClick={() => {
                  setIsOpen(false)
            ...
            {/* Admin Panel Link - Admins/owners only */}
            {hasGlobalAdminRole && (
              <button
                onClick={() => {
                  setIsOpen(false)
            ...
            {/* Health Dashboard Link - Admin/owner only (not moderators) */}
            {hasGlobalAdminRole && (
              <button
                onClick={() => {
                  setIsOpen(false)
