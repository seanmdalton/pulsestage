...
export function TeamManagement() {
  ...
  const handleDeactivateTeam = async (team: Team) => {
    if (!confirm(`Are you sure you want to deactivate the "${team.name}" team? This will hide it from the team selector.`)) {
      return;
    }
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Team "${team.name}" deactivated successfully!` });
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Team Management</h2>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      ...
      {showCreateForm && (
        <div className="mb-6 p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-900/50">
          ...
          <form onSubmit={handleCreateTeam} className="space-y-4">
            ...
            <div>
              <label htmlFor="team-slug" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                URL Slug *
              </label>
              <input
                id="team-slug"
            ...
            <div className="flex space-x-3">
              <button
                type="submit"
                disabled={!formData.name.trim() || !formData.slug.trim() || isCreating}
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">Existing Teams ({teams.length})</h3>
        
        {teams.length === 0 ? (
          <div className="text-center py-8">
            <div className="text-gray-500 dark:text-gray-400">No teams found</div>
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {teams.map((team) => (
              ...
              >
                ...
                {team.description && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{team.description}</p>
                )}
