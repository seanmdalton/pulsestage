...
test.describe('AMA Happy Path', () => {
  ...
  test('complete user journey: submit → open → upvote → admin respond → answered', async ({ page, request }) => {
    ...
    await expect(page.locator('text=Question submitted successfully!')).toBeVisible({ timeout: 5000 });

    // Step 2: Verify question was created via API
    const response = await request.get(`${API_URL}/questions?status=open`);
    const questions = await response.json();
    const ourQuestion = questions.find((q: any) => q.body === questionText);
    ...
    const upvoteResponse = await request.post(`${API_URL}/questions/${questionId}/upvote`);
    expect(upvoteResponse.ok()).toBeTruthy();
    
    // Verify upvote count increased
    const updatedResponse = await request.get(`${API_URL}/questions?status=open`);
    const updatedQuestions = await updatedResponse.json();
    const updatedQuestion = updatedQuestions.find((q: any) => q.id === questionId);
    ...
    const respondResponse = await request.post(`${API_URL}/questions/${questionId}/respond`, {
    ...
    const answeredResponse = await request.get(`${API_URL}/questions?status=answered`);
    const answeredQuestions = await answeredResponse.json();
    const answeredQuestion = answeredQuestions.find((q: any) => q.id === questionId);
  ...
  test('upvote button disables after click (localStorage guard)', async ({ page, context }) => {
  ...
  test('admin key is required for responding', async ({ request }) => {
    ...
    const unauthorizedResponse = await request.post(`${API_URL}/questions/${questionId}/respond`, {
      data: { response: 'Unauthorized response' }
    });
    expect(unauthorizedResponse.status()).toBe(401);
    
    // Try with correct admin key - should succeed
    const authorizedResponse = await request.post(`${API_URL}/questions/${questionId}/respond`, {
      headers: { 'x-admin-key': ADMIN_KEY },
